...
using glb_auto_perception_sensorfusion::util::GetProtoFromFile;
...
bool AsyncFusion::Init(bool log_fusion, std::string &file_path_config) {
  ...
  AWARN_IF(log_fusion_) << "Camera max invisible period: " 
    << "\033[1;33m" << config_.max_camera_invisible_period() << "\033[0m";
  AWARN_IF(log_fusion_) << "max_radar_confident_angle: " 
    << "\033[1;33m" << config_.max_radar_confident_angle() << "\033[0m";
  AWARN_IF(log_fusion_) << "min_radar_confident_distance: " 
    << "\033[1;33m" << config_.min_radar_confident_distance() << "\033[0m";
  return true;
}

// From SensorObjects convert to pbf_Sensorframe
...
void AsyncFusion::UpdateAssignedTracks(
  ...
  for (size_t i = 0; i < assignments.size(); ++i) {
    ...
    AWARN << "local track index " << local_track_index << " local object index "
          << local_obj_index;
...
void AsyncFusion::FuseForegroundObjects(
  ...
  // fixme: only create new track if it is lidar sensor
