...
def func1():
    ...
    with open('insMsg0401.txt') as f1, open('swap_a.txt', 'w') as f2:
        data = f1.read()
        datas = data.split('\n')
        count = 0
        for content in datas:
            # if 'Lat' in content or 'Lon' in content or 'Heading:' in content:
            if 'Lat' in content or 'Lon' in content or 'Heading:' in content or 'Veh_Spd:' in content:
                tep = content.split(':')
                count += 1
                # if count % 3 == 0:
                if count % 4 == 0:
...
def func2():
    ...
    os.remove('point.txt')
    os.rename('waypoint.csv.swap', 'waypoint0401.csv')
