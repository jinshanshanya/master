...
Lf = 4.57  # Forward looking distance
look_distance = 9
...
kappa_value =0.01
pi = np.pi
setsize = 100  # the unit length is 3.4cm,50point total 17m the setsize is averagely 8m from real wheel where the GPS-RTK locates; the parameter strongly affect the pure pursuit system performance;
# this number should not be less than 200;
path_waypoint = pd.read_csv("test0401.csv")
waypoint = path_waypoint.loc[:, ["x", "y", "heading",'kk']].values.astype(float)
way = pd.read_csv("waypoint0401.csv")
...
def ind_route_generation(current_x, current_y ,current_ind ,heading ,current_ind_F):
    global waypoint
    current_x_F = current_x +Lf*sin(heading*pi/180)
    current_y_F = current_y +Lf*cos(heading*pi/180)
    ...
    # Stanley  caculate need front axis "lateral_offset "
        ...
        p2_F = waypoint[current_ind_F+1,[0,1]]
    ...
    return  look_forward_point, current_ind,current_ind_F,lateral_offset ,current_x_F ,current_y_F
...
def breakthrough(current_x, current_y, look_forward_point, lateral_offset ,v):  # to find the inserting points:
    global look_distance
    look_dis = min((0.1*v+look_distance),17.5)
    #look_dis = look_distance +v*0
    #print("look_dis:",look_dis)
    #print("v:",v)
    current_x = 0
    current_y = 0
...
class PositionPID :
    def __init__(self):
        ...
        self.error_sum = 0
        self.error_past = 0
    def setTarget(self,target_v,kp,ki,kd):
        self.target_v = float(target_v)
        self.kp = float(kp)
        self.ki = float(ki)
        self.kd = float(kd)
        #return self.target_v,self.kp,self.ki,self.kd

    def updata(self,act_v):
        error = self.target_v - act_v
        self.error_sum +=error
        delta_error = error - self.error_past
        ...
        throat =p + i +d
        #print("error:",error)
        #print("error_past:",self.error_past)
        #print("delta_error:",delta_error)
        #print("error_sum:",self.error_sum)
        return throat


def control_steering(x_ref, y_ref,vehicle_length ,lateral_offset,delta_heading ,kk,current_ind_F,v):

    kappa=kk[current_ind_F]
    
    alpha = atan(x_ref / y_ref) 
 
    look_distance = np.sqrt(x_ref**2 + y_ref**2)    
             
    wheel_angle = atan(2 * vehicle_length * sin(alpha) / look_distance) * 180/pi

    pre_look = max(pre,v/3.6)

    lateral_error_angle = atan(lateral_offset*k_error/pre_look)*180/pi

    heading_error_angle = delta_heading*h_error

    if abs(kappa) > kappa_value :
        steering_wheel_angle = fd * (1*wheel_angle + 1*lateral_error_angle + 0*heading_error_angle) +2.2
    else :
        steering_wheel_angle = fd * (1*wheel_angle + 1*lateral_error_angle + 0*heading_error_angle) +2.2

    steering_wheel_angle = max(-40, min(steering_wheel_angle, 40))

    #print("pre_look:",pre_look)
...
def avoidance_judgment(current_ind,waypoint_x,waypoint_y,waypoint_heading,v_front,d,L):
    ...
    if v_front <= -10 and d <= 8 and d > 6 and complete == 1:
        i = 1
        dist = 0
        while dist <= 2 * 8 + L:
...
def obj_selection(current_x, current_y, heading, current_ind, raw_obj, ObjNum):
    global waypoint
    length = len(waypoint)
    distance_target = 0  # initialization for target distance
    distance_lon = 100  # initialization for shortest distance_target
    if length - current_ind < 250:  # set maximum search range as 250 points, which means 50m approximately.
        end_index = len(waypoint)
    else:
        end_index = current_ind + 250
    selection_point = waypoint[current_ind: end_index, [0, 1, 2]]
    #print('selection point is', selection_point)
    selection_point = global_to_frenet(current_x, current_y, heading,
                                       selection_point)  # convert selected waypoints to frenet coordinate
    # end if

    # for i in range(ObjNum):  # loop over every obstacle to find the lateral distance
        # pos_x = raw_obj[i].Rel_Pos[0] #pos info is [x,y,z] in each struct
        # pos_y = raw_obj[i].Rel_Pos[1]
        # pos_x = raw_obj.pos_x[i]
        # pos_y = raw_obj.pos_y[i]
    pos_x = raw_obj.x
    pos_y = raw_obj.y
    
    distance_matrix = selection_point[:, [0, 1]] - np.tile([pos_x, pos_y], (len(selection_point), 1))
    distance_norm = np.sqrt(np.diag(np.dot(distance_matrix, distance_matrix.T)))
    # print('dis norm is', distance_norm)
    perm = np.argsort(distance_norm)
    update_ind = perm[0]
    # print('update_ind is', update_ind)
    p1 = selection_point[update_ind, [0, 1]]
    if update_ind > len(selection_point) - 2:
        # It needs to be changed here. At the end of the day, we will not find a point. Let's join in and judge whether it's at the end
        p2 = selection_point[update_ind - 1, [0, 1]]
    else:
        p2 = selection_point[update_ind + 1, [0, 1]]
    target_point = np.array([pos_x, pos_y])
    # print('target point is', target_point)
    lateral_offset = find_distance(p1, p2, target_point)
    #print('target lateral off is', lateral_offset)
    # end for

    if abs(lateral_offset) <= 3:  # if distance is small enough, find its longitudinal distance
        # for i in range(update_ind):  # Optimize distance computing
        for i in range(1, update_ind):  # Optimize distance computing
            # distance_target = distance_target + np.sqrt(np.dot(selection_point[i, :].T, selection_point[i, :]))
            distance_target = distance_target + np.sqrt(np.dot((selection_point[i, :] - selection_point[i - 1, :]),
                                                               (selection_point[i, :] - selection_point[i - 1,
                                                                                        :]).T))
	   ...
	   distance_target = 100
        ...
        # end for
    # end if
    distance_lon = min(distance_lon, distance_target)  # update longitudinal distance
    # end for
    # print('distance_target is :', distance_target)
    return distance_lon
